...
def default_tui_config_path() -> str:
    explicit = os.environ.get("BILITERMINAL_CONFIG")
    if explicit:
        return explicit
    return str(Path.home() / ".biliterminal" / "config.toml")


def load_tui_config(path: str | None = None) -> TuiConfig:
    resolved = path or default_tui_config_path()
    config_path = Path(resolved)
    if not config_path.exists():
        return TuiConfig(path=str(config_path), mtime=None)
    try:
