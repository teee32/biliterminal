...
程序会获取 Bilibili 登录二维码，并自动在浏览器中打开。使用 Bilibili 手机 App 扫码并确认登录后，终端会自动保存登录凭据（`credentials.json`）到本地状态目录（`.omx/state/` 或 `~/.biliterminal/state/`）。

如果在交互式 REPL 中，也可以直接输入 `login` 启动扫码登录。
...
- 这是一个偏“轻量摸鱼”场景的终端浏览器，不是下载器，支持扫码和手动配置登录态以获取完整视频流，但没有实现投稿、评论发送、弹幕发送等其他交互写入功能。
